...
// ────────────────────────────────────────────────────────────
export function changeRequestApprovedTemplate(employeeName, dateStr, requestedShift, managerComment = '') {
  const formattedDate = new Date(dateStr).toLocaleDateString('en-US', {
    weekday: 'long', month: 'short', day: 'numeric', year: 'numeric'
  });

  const commentBlock = managerComment ? `
    <table width="100%" cellpadding="0" cellspacing="0" border="0" bgcolor="#0a1929" style="background-color: #0a1929; border-radius: 8px; margin: 24px 0; border-left: 4px solid #4fc3f7; border-top: 1px solid #1a365d; border-right: 1px solid #1a365d; border-bottom: 1px solid #1a365d;">
      <tr>
        <td style="padding: 16px;">
          <h4 style="margin: 0 0 8px 0; color: #4fc3f7; font-size: 13px; text-transform: uppercase; letter-spacing: 0.5px;">Manager Comment</h4>
          <p style="margin: 0; color: #b2bac2; font-size: 14px; line-height: 1.6; font-style: italic;">"${managerComment}"</p>
        </td>
      </tr>
    </table>
  ` : '';
        ...
        <td style="padding: 22px;">
          <table width="100%" cellpadding="0" cellspacing="0" border="0">
            <tr>
              ...
              <td style="padding: 10px 0; color: #ffffff; font-size: 13px; border-bottom: 1px solid #2a2a3d;">${formattedDate}</td>
            </tr>
            <tr>
              <td style="padding: 10px 0; color: #888888; font-size: 13px;">New Shift</td>
              <td style="padding: 10px 0; color: #4caf50; font-size: 14px; font-weight: 700; text-transform: capitalize;">${requestedShift}</td>
            </tr>
          </table>
        </td>
      </tr>
...
// ────────────────────────────────────────────────────────────
export function changeRequestRejectedTemplate(employeeName, dateStr, requestedShift, managerComment = '') {
  ...
  return wrapper(`
    ...
    <p style="color: #aaa; line-height: 1.6;">Hi <strong style="color: #fff;">${employeeName}</strong>,</p>
    <p style="color: #aaa; line-height: 1.6;">We regret to let you know that your recent schedule change request for <strong style="color: #fff;">${formattedDate}</strong> to <strong style="color: #fff;">${requestedShift}</strong> was <strong style="color: #e57373;">not approved</strong> by your manager at this time.</p>
